(function ($) {
    $.controls['TextInputPartial'] =
        ...
        <div class="input-wrap" width="{{inputWidth MaxLength}}px"><input id={{ModelId}}-{{Id}} type="text" class="input text_field" data-index={{index}} {{#if settings.labelAsPlaceholder}}placeholder="{{compile Name.template Name.data}}"{{/if}} value="{{Value}}"\
